...
function CmsBannerCreate() {
    ...
    <>
      <Head>
        <title>{t('LABELS.bannerCreate')}</title>
      </Head>

      <BannerDetails isCreate={true}/>
...
CmsBannerCreate.getLayout = (page) => {
  const { props } = page;
  return (
    <Authenticated session={props.children.props.session}>
      <Authorized session={props.children.props.session} scopes={["cms.write"]}>
...
export default CmsBannerCreate;
