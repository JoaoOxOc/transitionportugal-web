export declare const SIDEMENU_TRANSLATIONS: {
    readonly SIDEMENU_USER: {
        readonly profile: "Perfil";
        readonly inbox: "Caixa de Entrada";
        readonly projects: "Projectos";
        readonly posts: "Publicações";
        readonly eventsCalendar: "Calendário de Eventos";
        readonly messenger: "Mensagens";
        readonly logout: "Logout";
    };
    readonly SIDEMENU_HEADERS: {
        readonly general: "Geral";
        readonly content: "Conteúdo do Website";
        readonly management: "Administração";
        readonly Settings: "Definições";
    };
    readonly SIDEMENU_GENERAL: {
        readonly dashboard: "Dashboard";
    };
    readonly SIDEMENU_CONTENT: {
        readonly banner: "Banner";
        readonly about: "Sobre";
        readonly contacts: "Contactos";
        readonly circularEconomy: "Economia Circular";
        readonly events: "Eventos";
        readonly newsBlog: "Notícias/Publicações";
        readonly donations: "Donativos";
        readonly partners: "Parceiros";
    };
    readonly SIDEMENU_MANAGEMENT: {
        readonly users: "Utilizadores";
        readonly associations: "Associações/Entidades";
        readonly profiles: "Perfis";
        readonly scopes: "Permissões";
        readonly newsletter: "Subscrições da Newsletter";
        readonly privacy: "Política & Privacidade";
    };
    readonly SIDEMENU_SETTINGS: {
        readonly email: "Envio de Emails";
        readonly emailTemplates: "Templates de Email";
        readonly userAuth: "Definições de Autenticação";
        readonly clientApps: "Aplicações Cliente Autorizadas";
    };
};
//# sourceMappingURL=sidemenu.d.ts.map